        ...
        <template v-if="form['questions'].length">
          ...
          >
            <transition-group type="transition" :name="!dragging ? 'flip-list' : null">
              <div v-for="(item, i) in form['questions']" :key="item.id" class="flex mb-7 w-full">
                ...
                <div class="flex-1 py-5 px-7 rounded-lg bg-white" @click.prevent="active = i">
                  <span v-if="item.data['is_required'] && active !== i" class="mr-2 text-danger">*</span>
                  <input
                    :class="[
                      'mb-1 w-[min(80%,_600px)]',
                      {
                        'py-2.5 pl-3.5 border-b border-solid border-[#888] bg-[#eee]': active === i,
                        'font-bold': item.data.config['bold'],
                        italic: item.data.config['italic'],
                        underline: item.data.config['underline'],
                      },
                    ]"
                  ...
                  <div v-if="active === i" class="flex mb-3">
                    <div
                      :class="[
                        ...
                        item.data.config['bold'] ? 'bg-[#eee] text-[#555]' : 'text-[#888]',
                      ]"
                      role="button"
                      title="粗體"
                      @click.prevent="item.data.config['bold'] = !item.data.config['bold']"
                    >
                    ...
                    <div
                      :class="[
                        'flex items-center justify-center mr-1.5 w-9 h-9 rounded-md',
                        item.data.config['italic'] ? 'bg-[#eee] text-[#555]' : 'text-[#888]',
                      ]"
                      role="button"
                      title="斜體"
                      @click.prevent="item.data.config['italic'] = !item.data.config['italic']"
                    >
                    ...
                    <div
                      :class="[
                        'flex items-center justify-center mr-1.5 w-9 h-9 rounded-md',
                        item.data.config['underline'] ? 'bg-[#eee] text-[#555]' : 'text-[#888]',
                      ]"
                      role="button"
                      title="底線"
                      @click.prevent="item.data.config['underline'] = !item.data.config['underline']"
                    >
                    ...
                    <div
                      ...
                      title="移除文字效果"
                      @click.prevent="
                        ;(item.data.config['bold'] = false), (item.data.config['italic'] = false), (item.data.config['underline'] = false)
                      "
                    >
                  ...
                  <div>
                    ...
                    <template v-if="item.kind === 'input'">
                      ...
                      <div v-if="active === i" class="flex text-sm">
                        <select id="inputType" class="mr-5 p-2.5 w-1/5 hover:shadow-md" name="inputType" v-model="item.data.config['type']">
                        ...
                        <input
                          ...
                          v-model.number="item.data.config['max']"
                          type="number"
                        />
                      </div>
                    </template>
                    ...
                    <template v-else-if="item.kind === 'upload'">
                          ...
                          <div class="flex-1 w-4/5 2xl:w-1/2">
                            <div class="grid grid-flow-row grid-cols-3 lg:grid-cols-5 gap-3">
                              <div
                                ...
                                @click.prevent="mediaQuestion(item.data.config.file['type'], kind.value)"
                              >
                                <div
                                  :class="[
                                    'mr-3 w-5 h-5 border border-solid transition-all ease-in-out duration-500',
                                    item.data.config.file['type'].includes(kind.value) ? 'mediaChecked' : 'border-[#888]',
                                  ]"
                                />
                                <p>{{ kind.text }}</p>
                              </div>
                        ...
                        <div class="flex text-sm">
                          ...
                          <div>
                            <input
                              ...
                              v-model.number="item.data.config.file['size']"
                              type="number"
                              :max="5"
                            />
                      ...
                      <template v-else>
                        <div class="text-sm">
                          ...
                          <span class="text-[#888]">大小限制 {{ item.data.config.file['size'] }} MB</span>
                        </div>
                      </template>
                    </template>
                    <!-- 線性刻度 ↓ -->
                    ...
                    <div class="flex justify-end py-1">
                      <div class="flex">
                        ...
                        <div class="flex items-center py-3 px-4 border-l border-solid border-[#ccc]">
                          ...
                          <SwitchElement :status="item.data['is_required']" @switch_status="switch_status($event, item)" />
                        </div>
                      </div>
                    </div>
                  </template>
    ...
    handleDrop(env) {
      if (!this.dragging) {
        if (this.dragAdd) {
          ...
          this.form['questions'].push({ ...questionData, id: Date.now() })
        } else {
          /**@添加選中暫存選項 */
